...
$whoisservers = array(
	...
	"рф" => "whois.tcinet.ru", // Russian Federation
...
function LookupDomain($domain){
	...
	$result = QueryWhoisServer($whoisserver, $domain, $tld);
	if(!$result) {
...
function LookupDomainWhois($domain){
	global $whoisservers;
	$domain_parts = explode(".", $domain);
	$tld = strtolower(array_pop($domain_parts));
	$whoisserver = $whoisservers[$tld];
	...
	$result = QueryWhoisServerWhois($whoisserver, $domain);
	if(!$result) {
		return "Error: No results retrieved from $whoisserver server for $domain domain!";
	}
	else {
		while(strpos($result, "Whois Server:") !== FALSE){
			...
			$secondary = $matches[1];
			if($secondary) {
				$result = QueryWhoisServerWhois($secondary, $domain);
				$whoisserver = $secondary;
			}
		}
	}
...
function QueryWhoisServer($whoisserver, $domain, $tld) {
	$port = 43;
	$timeout = 10;
	...
	$res = "";
	if ($tld == 'com' || $tld == 'net'){
		if (strpos(strtolower($out), 'no match for "'.$domain.'"') !== FALSE)
			$res = 1;
	}
	if ($tld == 'ru' || $tld == 'рф'){
		if (strpos(strtolower($out), 'no entries found for the selected source') !== FALSE || strpos(strtolower($out), 'invalid request') !== FALSE)
			$res = 1;
	}
	if ($tld == 'ua'){
		//echo strtolower($out);
		if (strpos(strtolower($out), "no entries found for $domain") !== FALSE || strpos(strtolower($out), "domain not in database: $domain") !== FALSE || strpos(strtolower($out), "error") !== FALSE)
			$res = 1;
	}
	if ($tld == 'info' || $tld == 'org'){
		if (strpos(strtolower($out), "not found") !== FALSE)
			$res = 1;
	}
	if ($tld == 'name'){
		if (strpos(strtolower($out), "no match.") !== FALSE)
			$res = 1;
	}
	if ($tld == 'de'){
		if (strpos(strtolower($out), "status: free") !== FALSE)
			$res = 1;
	}
	if ($tld == 'biz'){
		if (strpos(strtolower($out), "not found: $domain") !== FALSE)
			$res = 1;
	}
...
function QueryWhoisServerWhois($whoisserver, $domain) {
	$port = 43;
	$timeout = 10;
	$fp = @fsockopen($whoisserver, $port, $errno, $errstr, $timeout) or die("Socket Error " . $errno . " - " . $errstr);
	//if($whoisserver == "whois.verisign-grs.com") $domain = "=".$domain; // whois.verisign-grs.com requires the equals sign ("=") or it returns any result containing the searched string.
...
if($domain) {
	...
	elseif(ValidateDomain($domain)) {
		$result = LookupDomainWhois($domain);
	}
	else die("Invalid Input!");
	echo "<pre>\n" . $result . "\n</pre>\n";
}
